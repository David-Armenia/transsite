...
<section class="pp-head">
  <div class="pp-head__grid">
    ...
    <aside class="pp-side" aria-label="Menu">
      <a class="pp-side__btn" href="#" aria-label="Profile"><span>👤</span></a>
      <a class="pp-side__btn" href="#" aria-label="Favorites"><span>♡</span></a>
      <a class="pp-side__btn" href="#" aria-label="Open public"><span>↗</span></a>
      <a class="pp-side__btn" href="#" aria-label="Settings"><span>⚙</span></a>
    </aside>

    <div class="pp-photo">
      <div class="pp-photo__media"><img src="<?php echo esc_url((string) $main_photo_url); ?>" alt=""></div>
      <button class="pp-photo__cta" type="button">Изменить фото</button>
...
<div class="pp-modal" id="ppMainPhotoModal" aria-hidden="true">
  ...
  <div class="pp-modal__panel" role="dialog" aria-modal="true">
    <div class="pp-modal__head">
      <div class="pp-modal__title">Выберите главное фото</div>
      <button class="pp-modal__close" type="button" data-pp-close>✕</button>
    </div>
    ...
    <form class="pp-mainphoto-form" method="post" action="<?php echo esc_url(admin_url("admin-post.php")); ?>">
      ...
      <div class="pp-modal__grid">
        <?php foreach ($gallery_ids as $att_id): ?>
          ...
          ?>
          <button type="button" class="pp-pick <?php echo ((int)$att_id === (int)$current_thumb_id) ? "is-current" : ""; ?>" data-att="<?php echo (int)$att_id; ?>" style="background-image:url('<?php echo esc_url($src); ?>');"></button>
        <?php endforeach; ?>
      </div>

      <?php if (empty($gallery_ids)): ?>
        <div class="pp-muted" style="margin-top:10px;">Сначала загрузите фото в Gallery.</div>
      <?php endif; ?>
    </form>
  </div>
</div>
...
(function(){
  ...
  modal.querySelectorAll(".pp-pick").forEach(function(b){
    ...
    <div class="pp-card pp-card--info">
      <h2 class="pp-card__title">Основная информация</h2>
      <div class="pp-fields">
        <div class="pp-field">
          <div class="pp-label">Имя</div>
          <div class="pp-value"><?php echo esc_html($current_user instanceof WP_User ? $current_user->display_name : ""); ?></div>
        </div>
        <div class="pp-field">
          <div class="pp-label">Номер телефона</div>
          <div class="pp-value"><?php echo esc_html($phone ?: "—"); ?></div>
        </div>
        <div class="pp-field">
          <div class="pp-label">Email</div>
          <div class="pp-value"><?php echo esc_html($email ?: "—"); ?></div>
        </div>
      </div>
      <a class="btn btn-dark pp-btn-wide" href="#" type="button">Редактировать</a>
    </div>

    <div class="pp-card pp-card--plan">
      <h2 class="pp-card__title">Тарифный план:</h2>
      ...
      <a class="btn btn-dark pp-btn-wide" href="#" type="button">Изменить</a>
    </div>
...
<section class="pp-active">
  <h2 class="pp-active__title">Активные анкеты</h2>
      ...
      <article class="pp-active-card">
        ...
        <div class="pp-active-card__media" style="background-image:url('<?php echo esc_url($p_thumb); ?>');">
          <?php if (!empty($p_price)): ?>
            <div class="pp-active-card__price">от <?php echo esc_html($p_price); ?></div>
          <?php endif; ?>
          <button class="pp-active-card__fav" type="button" aria-label="Favorite">♡</button>
        </div>
        <div class="pp-active-card__body">
  <div class="pp-active-stats">
    <span class="pp-stat"><i>❤️</i> 124</span>
    <span class="pp-stat"><i>👁</i> 1.2k</span>
    <span class="pp-stat"><i>⭐</i> 4.8</span>
  </div>

          <div class="pp-active-card__name"><?php echo esc_html($p_title ?: "—"); ?></div>
        </div>
      </article>
    <?php endforeach; ?>
    ...
    <a class="pp-active-add" href="<?php echo esc_url(home_url("/personal-profile/?create=1")); ?>">
      ...
      <span class="pp-active-add__text">Добавить анкету</span>
    </a>
  </div>
</section>
              ...
              <?php foreach ($gallery_ids as $att_id): ?>
                ...
                <div class="pp-gallery-item" style="background-image:url('<?php echo esc_url($url); ?>');">
                  ...
                  <a class="pp-gallery-open" href="<?php echo esc_url($href); ?>" target="_blank" rel="noopener"></a>
                    ...
                    <!-- set as main photo -->
                    <form method="post" class="pp-photo-main-form" action="<?php echo esc_url(admin_url("admin-post.php")); ?>">
                      <?php wp_nonce_field("transescort_set_main_photo", "transescort_set_main_photo_nonce"); ?>
                      <input type="hidden" name="action" value="transescort_set_main_photo">
                      <input type="hidden" name="profile_id" value="<?php echo (int)$profile_id; ?>">
                      <input type="hidden" name="att_id" value="<?php echo (int)$att_id; ?>">
                      <button type="submit" class="pp-photo-main-btn">Сделать главным</button>
                    </form>
