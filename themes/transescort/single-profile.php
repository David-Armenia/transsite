      ...
      ?>
          ...
          <div class="sp-top">
            <div class="sp-hero" data-te-lightbox="1" data-index="0">
              <?php if ($main_url): ?>
                <img class="sp-hero-img" loading="lazy" decoding="async" src="<?php echo esc_url($main_url); ?>" alt="">
              <?php endif; ?>
...
<!-- Gallery (Figma) -->
<section class="sp-block sp-gallery-block">
  <div class="profile-block-head">
    <h2 class="profile-block-title">Галерея</h2>
  </div>
  <?php if (!empty($gallery_ids)): ?>
    <div class="sp-gallery" data-te-gallery="1">
      <?php foreach ($gallery_ids as $gi => $gid): ?>
        <?php $gid = (int)$gid; if (!$gid) continue; ?>
        <?php $thumb = wp_get_attachment_image_url($gid, "medium"); $full = wp_get_attachment_image_url($gid, "full"); ?>
        <?php $src = $thumb ?: $full; if (!$src) continue; ?>
        <button type="button" class="sp-gitem" data-te-lightbox="1" data-index="<?php echo (int)$gi; ?>" style="background-image:url('<?php echo esc_url($src); ?>');"></button>
      <?php endforeach; ?>
    </div>
  <?php else: ?>
    <p class="pp-muted">Нет фото.</p>
  <?php endif; ?>
</section>
